...
Ext.onReady(function() {
                ...
                var store_checkin_related_data = Ext.create('Ext.data.Store', {
					...
					proxy: {
						type: 'ajax',
						url : '/checkin/yewuyuan/related_data.json',
						reader: {
							type: 'json',
							totalProperty  : 'total',
				...
				var store_checkin_table = Ext.create('Ext.data.Store', {
					...
					proxy: {
						type: 'ajax',
						url : '/checkin/yewuyuan/table.json',
						reader: {
							type: 'json',
							totalProperty  : 'total',
							successProperty: 'success'
                ...
                var stroe_checkin_names = Ext.create('Ext.data.Store', {
					...
					proxy: {
						type: 'ajax',
						url : '/checkin/yewuyuan/names.json',
						reader: {
							type: 'json',
							totalProperty  : 'total',
							successProperty: 'success'
...
//
				...
				var store_chart = Ext.create('Ext.data.Store', {
					...
					proxy: {
						type: 'ajax',
						url : '/checkin/yewuyuan/chart_data.json',
						reader: {
							type: 'json',
							totalProperty  : 'total',
							successProperty: 'success'
                ...
                var store_related_data_specific = Ext.create('Ext.data.Store', {
					...
					proxy: {
						type: 'ajax',
						url : '/checkin/yewuyuan/related_data_specific.json',
						reader: {
							type: 'json',
							totalProperty  : 'total',
							successProperty: 'success'
    ...
    checkin_chart = new Ext.Panel({
					...
					items: {
						...
						axes: [{
							...
							minimum: 0,
							maximum: 3800,
//                            maximum: parseFloat(store_chart.get('plan'))*1.05,
							position: 'left',
							fields: ['finish'],
							title: '应收额(万元)',
							grid: true,
...
//								renderer: Ext.util.Format.numberRenderer('0,0'),
						...
						series: [{
							...
							tips: {
								...
								height: 33,
								renderer: function(store_chart, item) {
									this.setTitle((store_chart.get('month') + '月计划' + (store_chart.get('plan')+'万元<br />完成比例：'+  (parseFloat((store_chart.get('finish'))/parseFloat((store_chart.get('plan')+0.0000001))).toFixed
                                        (2)*100 + '%'))));
								}
							},
							style: {
